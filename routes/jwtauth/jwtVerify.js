...
const jwtVerify = async (token, ignoreblock) => {
    ...
    try {
        ...
        if (ignoreblock && _isblock) {
            _response.tokenerror = "ERR_JWT_TOKEN_ACCESS_BLOCKED";
            return _response;
        }
        ...
        const _user = await User.findOne({"account.email" : _response.jwtdecode.account.email, "account.status": { $ne : "rejected"}}, {"auth":0, "profile":0});
        if (_user === null || _user === undefined) _response.tokenerror = "ERR_USER_ACCOUNT_NOT_ACCESSABLE";
