...
const jwtRefresh = async (req, accesstoken, refreshtoken) => {
    ...
    if (_refreshtoken.tokenerror !== null) {
        _response.tokenerror.refresherror = "ERR_JWT_REFRESH_TOKEN_INVALID";
    ...
    if (blockerror !== null) {
        _response.tokenerror.refresherror = "ERR_JWT_ACCESS_TOKEN_BLOCK_FAILED";
        _response.tokenerror.errordetail = blockerror;
        return _response;
    }

    //#REGENERATE ACCESS TOKEN WITH REFRESHTOKEN
    const _user = User.findOne({"account.email": _refreshtoken.jwtdecode.account.email,  "account.status": { $ne : "rejected"}}, {"auth":0, "_id":0});
    if (_user === null || _user === undefined || !_user) {
        _response.tokenerror.refresherror = "ERR_USER_ACCOUNT_NOT_ACCESSIBLE";
