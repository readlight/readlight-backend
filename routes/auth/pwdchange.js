...
router.put ("/:email", async (req,res) => {
    ...
    if (!(email && curpassword && newpassword)) return await responseFunction(res, 412, {"msg":"ERR_DATA_NOT_PROVIDED"}, null);
    if (!(emailchk.test(email) && passwdchk.test(curpassword) && passwdchk.test(newpassword))) return await responseFunction(res, 412, {"msg":"ERR_DATA_FORMAT_INVALID"}, null);
    else if (curpassword === newpassword) return await responseFunction(res, 424, {"msg":"ERR_USER_PASSWORD_DUPLICATE"}, null);
    ...
    const encryptCurPassword = await pbkdf2Sync(curpassword, _user.salt, 100000, 64, "SHA512");
    const encryptNewPassword = await pbkdf2Sync(newpassword, _user.salt, 100000, 64, "SHA512");
    if (!encryptCurPassword) return await responseFunction(res, 500, {"msg":"ERR_PASSWORD_ENCRYPT_FAILED"}, null, encryptCurPassword);
    if (!encryptNewPassword) return await responseFunction(res, 500, {"msg":"ERR_PASSWORD_ENCRYPT_FAILED"}, null, encryptNewPassword);
    ...
    if (encryptCurPassword.toString("base64") !== _user.password) return await SAVE_LOG(await responseFunction(res, 409, {"msg":"ERR_USER_AUTH_FAILED"}, null));
