...
router.post ("/", async (req,res) => {
    ...
    const _token = await Token.findOne({"owner" : email, "type" : "PWDRESET", "token" : token}, {"_id":0});
    ...
    const _verify = await User.updateOne({"account.email": email}, {
            "auth.denied": 0,
            "auth.password": encryptPassword.toString("base64"),
